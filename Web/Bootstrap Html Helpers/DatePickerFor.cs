...
namespace System.Web.Mvc.Html
{
    public static partial class HtmlHelper
    {
            ...
            (this HtmlHelper<TModel> self, Expression<Func<TModel, TValue>> expression, [Optional] params string[] DataAPIParameters)
        {
            ...
            var name = expression.Body.GetPropertyPathFromExpression();

            // Main input.
            var input = new TagBuilder("input");

            // Setting attributes.
            input.Attributes.Add("id", name.Replace(".", "_"));
            ...
            {
                if (DateTime.TryParse(metadata.Model.ToString(), out var value))
                {
                    input.Attributes.Add("value", value.ToShortDateString());
                }
                else
                {
